...
int main (int argc, char** argv) {
 ...
 ros::NodeHandle nh;
 //Subscribers
 ros::Subscriber sub_encoders = nh.subscribe ("/angle", 1, angles_cb);
 ros::Subscriber sub_foothold = nh.subscribe("/foothold_raw", 1, foothold_cb);
 ...
 ros::Subscriber sub_tilt = nh.subscribe("/tilt_angle", 1, tilt_cb);
 ...
 float camera_com_offs_z= 0.08; //vertical offset between camera and com (updated from 0.12 to 0.1 15/11/2023)
 ...
 while(ros::ok()){
	...
	if(new_data) { //condition should be new_data, put true to run continuously
		...
		float theta= calc_angle_from_side_sin(-heel_y, l);
		...
		//COM (HIP)
		l = euclidean_dist(0,cam_z, com_y, com_z);
		std::cout<<"Measured C-H segment: "<< sqrt(pow(camera_com_offs_y,2)+ pow(camera_com_offs_z,2))<<std::endl; 
		std::cout<<"Calculated C-H segment: "<< l<<std::endl; 
		theta= calc_angle_from_side_sin(-com_y, l);
		com_y = - l * sin(gamma+theta);
		com_z =cam_z - l * cos(gamma+theta);
		//SUPP KNEE
		l = euclidean_dist(0,cam_z, knee_y_p, knee_z_p);
		theta= calc_angle_from_side_sin(-knee_y_p, l);
		knee_y_p = - l * sin(gamma+theta);
		knee_z_p =cam_z - l * cos(gamma+theta);
		//SWING KNEE
		l = euclidean_dist(0,cam_z, knee_y, knee_z);
		theta= calc_angle_from_side_sin(-knee_y, l);
		knee_y = - l * sin(gamma+theta);
		knee_z =cam_z - l * cos(gamma+theta);
		//SWING FOOT
		l = euclidean_dist(0,cam_z, heel_y_p, heel_z_p);
		theta= calc_angle_from_side_sin(-heel_y_p, l);
		heel_y_p = - l * sin(gamma+theta);
		heel_z_p =cam_z - l * cos(gamma+theta);
		std::cout<<"Current swing ankle pos is: ("<< heel_y <<" , "<< heel_z << " )"<< std::endl; 
